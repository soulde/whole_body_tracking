...
class _MotionArgumentParser(argparse.ArgumentParser):
    def parse_args(self, args=None, namespace=None):
        ...
        if parsed.output_file is None:
            parsed.output_file = f"artifacts/motions/{parsed.output_name}/motion.npz"
        return parsed


def create_parser() -> argparse.ArgumentParser:
    """Create the preprocessing command-line parser without loading Isaac Sim."""
    parser = _MotionArgumentParser(description="Replay motion from csv file and output to npz file.")
    ...
    parser.add_argument(
        ...
        metavar=("START", "END"),
        help=(
            "frame range: START END (both inclusive). The frame index starts from 1. If not provided, all frames will"
            " be loaded."
        ),
    )
    parser.add_argument("--output_name", type=str, required=True, help="The name of the motion npz file.")
    parser.add_argument("--output_file", type=str, help="The local output path for the motion npz file.")
    parser.add_argument("--output_fps", type=int, default=50, help="The fps of the output motion.")
    parser.add_argument("--upload_wandb", action="store_true", help="Upload the saved motion to the WandB registry.")
    parser.add_argument("--overwrite", action="store_true", help="Overwrite an existing local output file.")
    return parser
    ...
    def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, joint_names: list[str]):
        ...
        while simulation_app.is_running():
            ...
            if reset_flag and not file_saved:
                ...
                digest = atomic_save_npz(output_path, log, overwrite=args_cli.overwrite)
                print(f"[INFO]: Motion saved to {output_path} (sha256: {digest})")
...
def main(argv=None):
    """Launch Isaac Sim and run the preprocessing workflow."""
